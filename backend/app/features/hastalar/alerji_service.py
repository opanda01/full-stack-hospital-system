...
def _assert_hasta_erisim(
    session: Session, current_user: Kullanici, hasta_id: int, kapsam: Kapsam
) -> None:
    if current_user.rol == Rol.ADMIN or kapsam == Kapsam.GLOBAL:
        return
    if current_user.rol == Rol.DOKTOR:
        ...
        return
    if current_user.rol in (Rol.HEMSIRE, Rol.EBE):
        allowed = hasta_service.hemsire_erisebilir_hasta_idler(session, current_user)
        if hasta_id not in allowed:
            raise HTTPException(status_code=403, detail="Hasta erişim yetkiniz yok")
        return
    if current_user.rol in (Rol.BASHEKIM, Rol.MUDUR, Rol.IDARI_PERSONEL):
        return
    raise HTTPException(status_code=403, detail="Alerji işlem yetkiniz yok")


def list_alerjiler(
    session: Session, current_user: Kullanici, public_id: UUID, kapsam: Kapsam
) -> list[HastaAlerjiRead]:
    h = hasta_from_public_id(session, public_id)
    assert h.id is not None
    _assert_hasta_erisim(session, current_user, h.id, kapsam)
    rows = session.exec(
